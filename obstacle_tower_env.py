...
class ObstacleTowerEnv(gym.Env):
    ...
    def __init__(
        ...
        if len(behavior_spec.action_shape) == 1:
            ...
            if flatten_branched:
                if reduced_actions:
                    self._flattener = ActionFlattener((2, 3, 2, 1))
                else:
                    self._flattener = ActionFlattener(behavior_spec.action_shape)
                self._action_space = self._flattener.action_space
            else:
                if reduced_actions:
                    self._action_space = spaces.MultiDiscrete((2, 3, 2, 1))
                else:
    ...
    def reset(self, config=None):
        ...
        # else:
        #     reset_params["tower-seed"] = random.randint(0, 95)
        #reset_params["default-theme"] = random.choice([2,4])
...
class ActionFlattener:
    ...
    def lookup_action(self, action):
        ...
        if isinstance(action, int):
            ...
            return self.action_lookup[tuple(action) if action.ndim > 0 else action.item()]
